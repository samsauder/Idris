package constants

object Descriptions {
    // Sam
    // Bouldering
    /// const val NEW_V7 = "Complete a never before seen V7 within four 60-min session."
    /// const val PROJECT_V7 = "Complete a never before seen V7 within four 60-min sessions."
    /// const val PROJECT_V8 = "Complete a never before seen V8 within four 60-min sessions."
    /// const val THREE_V5_NINE_V6 = "Complete 3 V5s and 9 V6s within one 90-min session."
    const val NEW_PURPLE = "Complete a never before seen Purple in one 60-min session."
    const val PROJECT_BLACK = "Complete a never before seen Black within four 60-min sessions."
    const val NEW_3X3_REDS = "Complete 3 sets of 3 reps of never before seen Reds in one 90-min session."
    const val NEW_4X3_REDS = "Complete 4 sets of 3 reps of never before seen Reds in one 90-min session."
    const val NEW_5X3_REDS = "Complete 5 sets of 3 reps of never before seen Reds in one 90-min session."
    const val NEW_7_REDS_120 = "Complete 7 new Reds in one 120-min session."

    const val PULLUP_85MAX = "Complete 5 sets of 3 reps of 85% 2RM weighted pullups (3m rests)."
    const val HANG_20MM_90MAX = "Complete 6 sets of 1 rep of 90% 1RM weighted hangs at 20mm (2m rests)."
    /// const val HANGING_KNEE_RAISES = "Complete 3 sets of 10 reps of hanging knee raises to chest."
    /// const val BENT_LEG_WINDSHIELDS = "Complete 3 sets of 10 reps (alternating sides) of bent leg windshield wipers."
    /// const val BENCH_PRESS = "Complete 3 sets of 5 reps of bench press (5min rests)."

    const val PULLUP_2RM_PLUS_50LB = "Achieve a +50 lb 2RM in weighted pullups."
    /// const val BENCH_1RM_80LB = "Achieve an 80 lb 1RM in bench press."

    // Chess
    const val COLOR_RECALL = "Recall the color of a random square in 3 seconds."
    const val STATIC_RECALL_100 = "Recall 100% of a random board position after 15 seconds of memorization."
    const val STATIC_RECALL_90 = "Recall 90%+ of a random board position after 15 seconds of memorization."
    const val STATIC_RECALL_80 = "Recall 80%+ of a random board position after 15 seconds of memorization."
    const val STATIC_RECALL_70 = "Recall 70%+ of a random board position after 15 seconds of memorization."

    const val PUZZLE_1M_2100 = "Solve a 2100 Lichess puzzle in under 1 minute."
    const val PUZZLE_1M_2200 = "Solve a 2200 Lichess puzzle in under 1 minute."
    const val PUZZLE_1M_2300 = "Solve a 2300 Lichess puzzle in under 1 minute."
    const val PUZZLE_1M_2500 = "Solve a 2500 Lichess puzzle in under 1 minute."

    const val RAPID_2000 = "Beat a 2000 Rapid player on Lichess."
    const val RAPID_2100 = "Beat a 2100 Rapid player on Lichess."
    const val RAPID_2200 = "Beat a 2200 Rapid player on Lichess."
    const val RAPID_2300 = "Beat a 2300 Rapid player on Lichess."

    const val GUESS_8PT6 = "Achieve an 8.6+ average Guess-the-Move score on ChessTempo."


    // Will
    // Karate
    const val FIVE_FORTY = "540 kick."
    const val REVERSE_CRESCENT = "Reverse crescent kick."
    const val TORNADO = "Tornado kick."
    const val SPINNING_TORNADO = "Spinning tornado kick."

    const val YELLOW_BELT_TEST = "Yellow belt test."
}